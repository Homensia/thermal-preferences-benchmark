...
### Full pipeline (seven commands, ~10–13 h on a single GPU)
...
# Figure — performance ceiling (three-panel: accuracy bars + Wilson CI, NN-agreement
# ceiling line, prior-weighted random baseline). Produced only by this script.
python3 analysis/performance_ceiling.py \
    --results_dir rerun_2026-04-28_no_reweighting/phase2_indomain \
    --output     rerun_2026-04-28_no_reweighting/regenerated_figures/performance_ceiling.pdf

# Post-processing — SUMMARY.md, audit_log.json, regenerated LaTeX tables, and the
# feature-importance heatmap (pdf/png/csv). Confusion matrices are already emitted
# per model by Phase 2 under phase2_indomain/<target>/<model>/.
...
### Figures — what the pipeline regenerates

| Paper figure | Regenerated by | Output location |
|---|---|---|
| Per-model confusion matrices (`<target>__<model>_confusion_matrix.png`) | Phase 2 (`models.py`) | `phase2_indomain/<target>/<model>/` |
| Performance ceiling (`performance_ceiling.pdf` + `_data.csv`) | `analysis/performance_ceiling.py` | `regenerated_figures/` |
| Feature-importance heatmap (`feature_importance_heatmap.{pdf,png,csv}`) | `analysis/feature_importance_heatmap.py` (via finalize) | `regenerated_figures/` |

The data-quality / vote-consistency figures of §2–§3 are produced by the two
notebooks in *Phase 4 – Data Distribution & Vote Consistency Analyses* above
(run them after `models.py`). The remaining manuscript figures are **conceptual
schematics** (`pipeline.png`, the fine-tune / `tab-vs-ft` diagrams, drawn in
draw.io) and **legacy per-model importance exports** (`*_feature_importance_LI.jpg`,
superseded by the heatmap above) — these are *not* regenerated by the pipeline.
